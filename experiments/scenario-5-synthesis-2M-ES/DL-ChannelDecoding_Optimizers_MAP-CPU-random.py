...
for code in codes:
    ...
    for nb_epoch in nb_epochs:
        ...
        patience_limit = np.round(np.sqrt(nb_epoch)).astype(int)
        
        dump['exec'][code][nb_epoch]['patience'] = str(patience_limit)
        ...
        for optimizer in optimizers:
            ...
            early_stopping = EarlyStopping(monitor='loss', patience=patience_limit, restore_best_weights=True)

            start_time = timeit.default_timer()
            print(f'{datetime.now()}\t{code} @ {optimizer} @ Mep=2^{nb_epoch_exp} fit started.')
            history = model.fit(x, d, batch_size=batch_size, epochs=nb_epoch, verbose=0, shuffle=True, callbacks=[early_stopping])
            ...
            nb_epoch_actual = len(history.history['loss'])
            dump['exec'][code][nb_epoch][optimizer]['actual_epochs'] = nb_epoch_actual
            # dump['exec'][code][nb_epoch][optimizer]['history'] = history.history            
            
            fig_opt, ax_opt = plt.subplots(1, 1, figsize=(11.75,8.25))
            ax_opt.plot(np.arange(0,nb_epoch_actual), history.history['loss'], label='Loss', color='red')
            ax_opt.plot(np.arange(0,nb_epoch_actual), history.history['ber'], label='BER', color='blue')
            ax_opt.grid(True, which='both')
            ax_opt.legend(loc='upper right')
            ax_opt.set_xlabel('Epoch')
            ax_opt.set_ylabel('Value')
            ax_opt.set_xlim(0, nb_epoch_actual)
            ax_opt.set_ylim(0, 1)
            ...
            ax_ans.semilogy(x_nn, y_nn, label=optimizer.capitalize(), linestyle='solid', linewidth=2.0)
