...
exponents = np.linspace(10, 18, num=5)       # exponents of learning epochs (n)
...
for code in codes:
    ...
    for nb_epoch in nb_epochs:
        ...
        for optimizer in optimizers:
            ...
            test_batch = 1000  
            num_words = 200000      # multiple of test_batch (era 2000000)
        ...
        plt.savefig(f'gpu_code={code}_epochs=2^{nb_epoch_exp}_map.png')
        # plt.show()
        ...
        with open(f'gpu_code={code}_epochs=2^{nb_epoch_exp}_map.png', 'rb') as image_file:
...
with open(f"gpu_codes=[{','.join(codes)}]_results_map.json", 'w') as outfile:
